...
| `{{> partial}}` | NTTP render の entry pair から外部断片を注入 | ヘッダー/フッターの挿入 |

**推奨**: 
- 両方の API で `#partialdef` / `#partial` が使えます。`#partialdef` は同一テンプレート内で定義し、`{{> name}}` は外部 entry pair から本文を持ってきます（内部は同じ partial メカニズム）。
