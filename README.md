...
| 構文 | 使用場面 | 例 |
|------|----------|-----|
| `{{#partialdef name}}...{{/partialdef}}` | engine で名前付き partial を定義 | 共通UI部品の定義 |
| `{{#partial name}}` | 定義済み partial を描画 | サイドバーの描画 |
| `{{> partial}}` | NTTP で partial を展開 | ヘッダー/フッターの挿入 |

**推奨**: 
- **engine**: 複雑なUI部品の再利用に適しています。HTMX等の部分更新にも使用できます。
- **NTTP**: シンプルなテンプレート断片の挿入に適しています。
