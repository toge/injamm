#pragma once
#ifndef RENDER_RENDER_ROUND_NOARG_HPP
#define RENDER_RENDER_ROUND_NOARG_HPP
/**
 * @file render.hpp
...
render_round_noarg(const T& data, Sink& out) {
  ...
  if constexpr (::injamm::detail::serializable_v<decltype(data.total)>) {
    ...
    _filtered.assign((data.total).data(), (data.total).size());
  }
  filter_float_round(_filtered, 0);
...
render_round_noarg(const T& data) {
  std::string out;
  auto result = render_round_noarg(data, out);
...
#endif // RENDER_RENDER_ROUND_NOARG_HPP
