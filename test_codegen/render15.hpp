#pragma once
#ifndef RENDER_RENDER15_HPP
#define RENDER_RENDER15_HPP
/**
 * @file render.hpp
...
render15(const T& data, std::string& out) {
  out.clear();
  out.reserve(1);
  
  html_escape_append_value(out, data.primary_item);
  out += "|";
  append_value(out, data.primary_item);
...
render15(const T& data) {
  std::string out;
  auto result = render15(data, out);
