#pragma once
#ifndef RENDER_RENDER_SORT_JOIN_HPP
#define RENDER_RENDER_SORT_JOIN_HPP
/**
 * @file render.hpp
...
render_sort_join(const T& data, Sink& out) {
  if constexpr (std::is_same_v<Sink, std::string>) {
    out.clear();
    out.reserve(34);
  }
  
  auto _size1 = data.sitems.size();
  std::size_t _lo1 = 0, _hi1 = _size1;
  bool _bwd1 = false;
  using _elem_t1 = std::remove_cvref_t<decltype((data.sitems)[0])>;
  std::vector<_elem_t1> _sorted1;
  if (_size1 > 0) {
    _sorted1.assign((data.sitems).begin(), (data.sitems).end());
...
render_sort_join(const T& data) {
  std::string out;
  auto result = render_sort_join(data, out);
...
#endif // RENDER_RENDER_SORT_JOIN_HPP
