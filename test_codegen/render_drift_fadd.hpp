#pragma once
#ifndef RENDER_RENDER_DRIFT_FADD_HPP
#define RENDER_RENDER_DRIFT_FADD_HPP
/**
 * @file render.hpp
...
render_drift_fadd(const T& data, Sink& out) {
  ...
  if constexpr (::injamm::detail::serializable_v<decltype(data.score)>) {
    _filtered.clear(); ::injamm::detail::serialize_value(_filtered, data.score);
  } else if constexpr (::injamm::detail::ct_glz_reflectable<decltype(data.score)>) {
    (void)::glz::write_json(data.score, _filtered);
  } else {
    _filtered.assign(data.score);
  }
  filter_int_add(_filtered, 1);
...
render_drift_fadd(const T& data) {
  std::string out;
  auto result = render_drift_fadd(data, out);
...
#endif // RENDER_RENDER_DRIFT_FADD_HPP
