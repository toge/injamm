#pragma once
#ifndef RENDER_RENDER_SORT_TAKE_HPP
#define RENDER_RENDER_SORT_TAKE_HPP
/**
 * @file render.hpp
...
render_sort_take(const T& data, Sink& out) {
  if constexpr (std::is_same_v<Sink, std::string>) {
    out.clear();
    out.reserve(33);
  }
  
  auto _size1 = data.iitems.size();
  std::size_t _lo1 = 0, _hi1 = _size1;
  bool _bwd1 = false;
  if (_bwd1) _lo1 = (_hi1 > 2u ? std::max(_lo1, _hi1 - 2u) : _lo1);
  else _hi1 = std::min(_hi1, _lo1 + 2u);
  using _elem_t1 = std::remove_cvref_t<decltype((data.iitems)[0])>;
  std::vector<_elem_t1> _sorted1;
  if (_size1 > 0) {
    _sorted1.assign((data.iitems).begin(), (data.iitems).end());
    std::stable_sort(_sorted1.begin(), _sorted1.end(), std::less<_elem_t1>{});
  }
  auto _count1 = _hi1 - _lo1;
  for (std::size_t _i1 = 0; _i1 < _count1; ++_i1) {
    auto _idx1 = _i1;
    const auto& _item1 = _sorted1[_idx1];
...
render_sort_take(const T& data) {
  std::string out;
  auto result = render_sort_take(data, out);
...
#endif // RENDER_RENDER_SORT_TAKE_HPP
